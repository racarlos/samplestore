...
export default function ProductCard({ product }: ProductCardProps) {
  ...
  return (
    <View className="w-[48%] bg-white p-2 rounded-lg border border-gray-200 mb-4">
      <Link href={`/product/${product.id}`} asChild>
        <TouchableOpacity>
          <Image source={{ uri: product.image }} className="w-full h-36 rounded-lg mb-3" />
          <Text className="text-gray-900 font-medium">{product.productName}</Text>
      ...
      {cartQuantity === 0 ? (
        <TouchableOpacity 
          className="bg-blue-600 py-2 rounded-lg mt-2" 
